...
class BeamTreeEnsemble(torch.nn.Module):
    def __init__(self, tree_parameters, n_features, n_classes):
        ...
        self.num_nodes = max([len(tree_parameter[2]) for tree_parameter in tree_parameters])
...
class BeamPPTreeEnsemble(torch.nn.Module):
    def __init__(self, tree_parameters, n_features, n_classes):
        ...
        self.leaf_nodes = torch.nn.Parameter(
            torch.from_numpy(weight_1.reshape((-1, n_classes)).astype("float32")), requires_grad=False
        )
