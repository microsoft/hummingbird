...
class CountVectorizer(BaseOperator, torch.nn.Module):
    """
        Class implementing CountVectorizer operator in PyTorch.
    """

    def __init__(self, vocabulary, max_features, stop_words, max_word_length, device):
        super(CountVectorizer, self).__init__(transformer=True)
        self.max_word_length = max_word_length
        self.original_vocab_size = max_features
        self.voabulary = vocabulary
    ...
    def forward(self, documents):
        # type: (List[Tensor]) -> Tensor  # noqa: F821
