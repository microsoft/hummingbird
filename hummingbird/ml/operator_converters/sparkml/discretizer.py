...
def convert_sparkml_bucketizer(operator, device, extra_config):
    ...
    labels = []
    input_indices = [i for i in range(len(operator.inputs)) if operator.inputs[i].full_name == operator.raw_operator.getInputCol()]
    ...
    # change the name of the output of this op
    operator.outputs[-1].name = operator.raw_operator.getOutputCol()
