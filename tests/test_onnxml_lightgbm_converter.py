...
class TestONNXLightGBMConverter(unittest.TestCase):
    ...
    # # Check that ONNXML models can also target torch backend.
    # @unittest.skipIf(
    #     not (onnx_ml_tools_installed() and onnx_runtime_installed()), reason="ONNXML test require ONNX, ORT and ONNXMLTOOLS"
    # )
    # @unittest.skipIf(not lightgbm_installed(), reason="LightGBM test requires LightGBM installed")
    ...
    # The random forest with LightGBM
    @unittest.skipIf(
        not (onnx_ml_tools_installed() and onnx_runtime_installed()), reason="ONNXML test require ONNX, ORT and ONNXMLTOOLS"
    )
    @unittest.skipIf(not lightgbm_installed(), reason="LightGBM test requires LightGBM installed")
    def test_rf_lightgbm_classifier(self):
        ...
        onnx_ml_model = convert_lightgbm(model, initial_types=initial_types, target_opset=9)  # noqa
        onnx_model = convert(onnx_ml_model, "onnx", X)  # noqa
